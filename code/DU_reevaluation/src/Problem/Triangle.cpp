...
//    cout << "Reading input data." << endl;
    ...
    rdm_tseries_dir = "/scratch/04528/tg838274/JLWTP_2021/paper3/DU_reeval/WaterPaths/RDM_inflows_demands/RDM_" + to_string(rdm_no) + BAR;

#pragma omp parallel num_threads(omp_get_thread_num())
    {
#pragma omp single
        streamflows_durham = Utils::parse2DCsvFile(
                rdm_tseries_dir + "final_synthetic_inflows" +
...
#pragma omp single
        evap_durham = Utils::parse2DCsvFile(
                rdm_tseries_dir + "final_synthetic_inflows/evaporation" +
...
#pragma omp single
        demand_cary = Utils::parse2DCsvFile(
                rdm_tseries_dir + "synthetic_demands_pwl" + BAR
                + "cary_synthetic_demands.csv", n_realizations);
#pragma omp single
        demand_durham = Utils::parse2DCsvFile(
                rdm_tseries_dir + "synthetic_demands_pwl" + BAR 
                + "durham_synthetic_demands.csv", n_realizations);
#pragma omp single
        demand_raleigh = Utils::parse2DCsvFile(
                rdm_tseries_dir + "synthetic_demands_pwl" + BAR 
                + "raleigh_synthetic_demands.csv", n_realizations);
#pragma omp single
        demand_owasa = Utils::parse2DCsvFile(
                rdm_tseries_dir + "synthetic_demands_pwl" + BAR
                + "owasa_synthetic_demands.csv", n_realizations);
#pragma omp single
        demand_pittsboro = Utils::parse2DCsvFile(
                rdm_tseries_dir + "synthetic_demands_pwl" + BAR
                + "pittsboro_synthetic_demands.csv", n_realizations);
#pragma omp single
        demand_chatham = Utils::parse2DCsvFile(
                rdm_tseries_dir + "synthetic_demands_pwl" + BAR 
		+ "chatham_synthetic_demands.csv", n_realizations);
