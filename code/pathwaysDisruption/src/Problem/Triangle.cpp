...
//                        added_capacity_sanford_wtp_intake_pittsboro_expansion_high);
        ...
        double added_capacity_sanford_wtp_stake_cary = 10.0 * 7*vars[110];
        ...
        //if (sum_wjlwtp > 1) {
        // Jan 2022: Normalize no matter what, don't want to build a fraction of a plant
...
//        vector<double> fl_treatment_allocation_fractions = {0.0, 0.0, 0.0, 1.0};
    ...
    //                         &evaporation_falls_lake, &falls_lake_storage_area);
        ...
        int little_river_reservoir_raleigh_permitting_period = 17 + vars[71]; // assume 2035 from TRWSP 2014 report
        int richland_creek_quarry_permitting_period = 40 + vars[73]; // from Falls Lake re-alloc report USACE (2017): not ready until at least 2057
        int falls_lake_reallocation_permitting_period = 7 + vars[91]; // based on TRWSP 2014, assume 2025 availability
        int teer_quarry_permitting_period = 7 + vars[75]; // 2025
        int neuse_river_intake_permitting_period = 17 + vars[77]; // assume 2035 availability, TRWSP 2014
        int haw_river_intake_low_permitting_period = 2 + vars[99]; // earliest by 2023 (Pittsboro 2019 report), say 2020 (2014 JLA report implies 2020)
        int haw_river_intake_high_permitting_period = 5 + vars[101]; // earliest by 2023, though older report says 2025
        int cape_fear_river_permitting_period = 17 + vars[79]; // for Chatham County, no longer a viable option according to Hazen and Sawyer, but switched it to a possibility for Cary by 2035
        int sanford_wtp_intake_low_permitting_period = 7 + vars[103]; // should be 7 (2025)
        int sanford_wtp_intake_high_permitting_period = 13 + vars[105]; // should be 22 (2040), 2014 Chatham report says 2031 is earliest for a harnett county project so use that...
        int cane_creek_reservoir_expansion_permitting_period = 17 + vars[97]; // 2035
        int stone_quarry_low_permitting_period = 22 + vars[81]; // 2040
        int stone_quarry_high_permitting_period = 22 + vars[83]; // 2040
        int university_lake_expansion_permitting_period = 17 + vars[85]; // 2035
        int lake_michie_low_permitting_period = 17 +vars[87]; // 2035
        int lake_michie_high_permitting_period = 17 + vars[89]; // 2035
        int reclaimed_low_permitting_period = 7 + vars[93]; // 2025
        int reclaimed_high_permitting_period = 7 + vars[95]; // 2025
        int dummy_wjlwtp_permitting_period = 100; // just a dummy, never built
        int wjlwtp_low_permitting_period = 5 + vars[107]; // Pittsboro 2019 report says 2035, TRWSP (2014) report says 2020-2025, so set to 2023 (if began today...)
        int wjlwtp_high_permitting_period = 7 + vars[109]; // 2019 report says 2040, TRWSP (2014) says between 2020-2025, Pittsboro 2014 report says 2030... say 2025
        int sanford_stake_cary_permitting_period = 0 + vars[111]; // can be added now, Cary has interconnection infrastructure already
        ...
        LevelDebtServiceBond lrr_bond(little_river_raleigh_reservoir_id, 263.0*vars[70], 25, 0.05, vector<int>(1, 0));
        Reservoir little_river_reservoir("Little River Reservoir (Raleigh)", little_river_raleigh_reservoir_id, catchment_little_river_raleigh, 3700.0*vars[70],
        ...
        LevelDebtServiceBond rcq_bond(richland_creek_quarry_id, 400.0*vars[72], 25, 0.05, vector<int>(1, 0));
        Quarry richland_creek_quarry("Richland Creek Quarry", richland_creek_quarry_id, gage_clayton, 4000.0*vars[72], ILLIMITED_TREATMENT_CAPACITY,
                                     evaporation_falls_lake, 100., construction_time_interval,
                                     richland_creek_quarry_permitting_period * WEEKS_IN_YEAR, rcq_bond,
                                     50. * 7);

        BalloonPaymentBond tq_bond(teer_quarry_id, 22.6*vars[74], 25, 0.05, vector<int>(1, 0), 3);
        Quarry teer_quarry("Teer Quarry", teer_quarry_id, vector<Catchment *>(), 1315.0*vars[74], ILLIMITED_TREATMENT_CAPACITY,
        ...
        LevelDebtServiceBond neuse_bond(neuse_river_intake_id, 225.5*vars[76], 25, 0.05, vector<int>(1, 0));
        Intake neuse_river_intake("Neuse River Intake", neuse_river_intake_id, vector<Catchment *>(), vector<int>(),
                16 * 7*vars[76], construction_time_interval,
        ...
        LevelDebtServiceBond low_haw_river_intake_expansion_bond(haw_river_intake_expansion_low_id, 20*0.93*vars[98], 25, 0.05, vector<int>(1, 0), 3);
        IntakeExpansion haw_river_intake_expansion_low("Haw River Intake Low Expansion", haw_river_intake_expansion_low_id, haw_river_intake_id,
                                                       added_capacity_haw_river_intake_expansion_low*vars[98], added_capacity_haw_river_intake_expansion_low*vars[98],
                                                       construction_time_interval,
        ...
        LevelDebtServiceBond high_haw_river_intake_expansion_bond(haw_river_intake_expansion_high_id, 30*0.93*vars[100], 25, 0.05, vector<int>(1, 0), 3);
        IntakeExpansion haw_river_intake_expansion_high("Haw River Intake High Expansion", haw_river_intake_expansion_high_id, haw_river_intake_id,
                                                        added_capacity_haw_river_intake_expansion_high*vars[100], added_capacity_haw_river_intake_expansion_high*vars[100],
                                                        construction_time_interval,
                                                        {haw_river_intake_expansion_low_id, haw_river_intake_expansion_high_id},
        ...
        LevelDebtServiceBond cape_fear_river_intake_bond(harnett_county_intake_id, 221.43*vars[78], 25, 0.05, vector<int>(1, 0), 3);
        Intake cape_fear_river_intake("Cape Fear River Intake", harnett_county_intake_id, vector<Catchment *>(),
                vector<int>(), 12.2 * 7*vars[78], construction_time_interval,
        ...
        vector<double> sanford_expansion_low_costs = {0.0, chatham_low_cost*vars[102], pittsboro_low_cost*vars[102]};
        vector<double> sanford_expansion_high_costs = {0.0, chatham_high_cost*vars[104], pittsboro_high_cost*vars[104]};
        vector<double> sanford_allocated_low_supply_expansions =
                {0.0,
                 added_capacity_sanford_wtp_intake_pittsboro_expansion_low*vars[102],
                 added_capacity_sanford_wtp_intake_chatham_expansion_low*vars[102]};
        ...
        vector<double> sanford_allocated_high_supply_expansions =
                {0.0,
                 added_capacity_sanford_wtp_intake_pittsboro_expansion_high*vars[104],
                 added_capacity_sanford_wtp_intake_chatham_expansion_high*vars[104]};
        ...
        double cary_stake_cost = (3.3+7.9)*0.94 * 5*vars[110]; // in millions, capital cost
        double cary_stake_allocation = 10.0 * 7*vars[110];
        ...
        falls_lake_reallocation = 5637.0*vars[90]; // in MG
        ...
        LevelDebtServiceBond fl_bond(falls_lake_reallocation_id, 142.0*vars[90], 25, 0.05, vector<int>(1, 0));
        ...
        LevelDebtServiceBond ccr_exp_bond(cane_creek_reservoir_expansion_id, 127.0*vars[96], 25, 0.05, vector<int>(1, 0));
        ReservoirExpansion ccr_expansion("Cane Creek Reservoir Expansion", cane_creek_reservoir_expansion_id, cane_creek_reservoir_id, 3000.0*vars[96], construction_time_interval,
                                         cane_creek_reservoir_expansion_permitting_period * WEEKS_IN_YEAR, ccr_exp_bond);

        LevelDebtServiceBond low_sq_exp_bond(stone_quarry_expansion_low_id, 1.4*vars[80], 25, 0.05, vector<int>(1, 0));
        ReservoirExpansion low_sq_expansion("Low Stone Quarry Expansion", stone_quarry_expansion_low_id, stone_quarry_id, 1500.0*vars[80], construction_time_interval,
                                            stone_quarry_low_permitting_period * WEEKS_IN_YEAR, low_sq_exp_bond);

        LevelDebtServiceBond high_sq_exp_bond(stone_quarry_expansion_high_id, 64.6*vars[82], 25, 0.05, vector<int>(1, 0));
        ReservoirExpansion high_sq_expansion("High Stone Quarry Expansion", stone_quarry_expansion_high_id, stone_quarry_id, 2200.0*vars[82], construction_time_interval,
                                             stone_quarry_high_permitting_period * WEEKS_IN_YEAR, high_sq_exp_bond);

        LevelDebtServiceBond ul_exp_bond(university_lake_expansion_id, 107.0*vars[84], 25, 0.05, vector<int>(1, 0));
        ReservoirExpansion univ_lake_expansion("University Lake Expansion", university_lake_expansion_id, university_lake_id, 2550.0*vars[84], construction_time_interval,
                                               university_lake_expansion_permitting_period * WEEKS_IN_YEAR, ul_exp_bond);

        LevelDebtServiceBond low_mi_exp_bond(michie_expansion_low_id, 158.3*vars[86], 25, 0.05, vector<int>(1, 0));
        ReservoirExpansion low_michie_expansion("Low Lake Michie Expansion", michie_expansion_low_id, durham_reservoirs_id, added_storage_michie_expansion_low*vars[86],
                                                construction_time_interval,
                                                lake_michie_low_permitting_period * WEEKS_IN_YEAR, low_mi_exp_bond);

        LevelDebtServiceBond high_mi_exp_bond(michie_expansion_high_id, 203.3*vars[88], 25, 0.05, vector<int>(1, 0));
        ReservoirExpansion high_michie_expansion("High Lake Michie Expansion", michie_expansion_high_id, durham_reservoirs_id, added_storage_michie_expansion_high*vars[88],
                                                 construction_time_interval,
                                                 lake_michie_high_permitting_period * WEEKS_IN_YEAR, high_mi_exp_bond);

        LevelDebtServiceBond low_rec_bond(reclaimed_water_low_id, 27.5*vars[92], 25, 0.05, vector<int>(1, 0));
        WaterReuse low_reclaimed("Low Reclaimed Water System", reclaimed_water_low_id, reclaimed_capacity_low*vars[92], construction_time_interval,
                                 reclaimed_low_permitting_period * WEEKS_IN_YEAR, low_rec_bond);

        LevelDebtServiceBond high_rec_bond(reclaimed_water_high_id, 104.4*vars[94], 25, 0.05, vector<int>(1, 0));
        WaterReuse high_reclaimed("High Reclaimed Water System", reclaimed_water_high_id, reclaimed_capacity_high*vars[94], construction_time_interval,
