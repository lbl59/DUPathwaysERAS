...
#ifdef PARALLEL
void RaleighDefection::setProblemDefinition(BORG_Problem &problem)
{
    // The parameter bounds are the same for all formulations
    BORG_Problem_set_bounds(problem, 0, 0.001, 1.0); // Raleigh restrictions
    BORG_Problem_set_bounds(problem, 1, 0.001, 1.0); // Raleigh Transfers
    BORG_Problem_set_bounds(problem, 2, 0.0, 0.1);  // Raleigh reserve fund cont
    BORG_Problem_set_bounds(problem, 3, 0.001, 1.0);  // Raleigh infrastructure trigger
    BORG_Problem_set_bounds(problem, 4, 0.0, 1.0);   // Little River Raleigh ranking
    BORG_Problem_set_bounds(problem, 5, 0.0, 1.0);  // Richland Creek ranking
    BORG_Problem_set_bounds(problem, 6, 0.0, 1.0);  // Neuse River Ranking
    BORG_Problem_set_bounds(problem, 7, 0.0, 1.0);  // Falls Lake Reallocation Ranking
    //BORG_Problem_set_bounds(problem, 11, 0.001, 0.2);  // WJWLTP OWASA frac
    BORG_Problem_set_bounds(problem, 8, 0.0, 20.0);  // Raleigh inf buffer
 ...
 * infrastructure).
 */
int RaleighDefection::functionEvaluation(double *vars, double *objs, double *consts) {
//    cout << "Building Triangle Problem." << endl;
    ...
    double raleigh_restriction_trigger = vars[0];
    double raleigh_transfer_trigger = vars[1];
    double raleigh_annual_payment = vars[2];
    double raleigh_inftrigger = vars[3];
    double little_river_reservoir_ranking = vars[4]; // 7
    double richland_creek_quarry_rank = vars[5]; // 8
    double neuse_river_intake_rank = vars[6]; // 10
    double reallocate_falls_lake_rank = vars[7]; // 17
    double raleigh_inf_buffer = vars[8];
...
//	printf("Function evaluation time: %f s\n", end_time - start_time);
    ...
    /// Calculate objectives and store them in Borg decision variables array.
        ...
        objs[0] = 1.0- objectives[18];
        objs[1] = objectives[19]; 
        objs[2] = objectives[20];
        objs[3] = objectives[23];
        objs[4] = objectives[22];
        objs[5] = objectives[23];
...
int RaleighDefection::simulationExceptionHander(const std::exception &e, Simulation *s,
...
RaleighDefection::~RaleighDefection() = default;

RaleighDefection::RaleighDefection(unsigned long n_weeks, int import_export_rof_table)
        : Problem(n_weeks) {
...
void RaleighDefection::readInputData() {
