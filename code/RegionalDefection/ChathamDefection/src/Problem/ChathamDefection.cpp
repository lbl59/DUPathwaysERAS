...
#ifdef PARALLEL
void ChathamDefection::setProblemDefinition(BORG_Problem &problem)
{
    // The parameter bounds are the same for all formulations
    BORG_Problem_set_bounds(problem, 0, 0.001, 1.0); // chatham restriction trigger
    BORG_Problem_set_bounds(problem, 1, 0.001, 1.0); // chatham transfer trigger
    BORG_Problem_set_bounds(problem, 2, 0.06, 0.35); // chatham JLA
    BORG_Problem_set_bounds(problem, 3, 0.0, 0.1); // chatham annual payment
    BORG_Problem_set_bounds(problem, 4, 0.001, 1.0); // chatham inf trigger
    BORG_Problem_set_bounds(problem, 5, 0.0, 20.0); // chatham inf buff
    //BORG_Problem_set_bounds(problem, 6, 0.01, 0.5); // wjlwtp chatham frac
    BORG_Problem_set_bounds(problem, 6, 0.0, 1.0); // wjlwtp rank low chatham
    BORG_Problem_set_bounds(problem, 7, 0.0, 1.0); // wjlwtp rank high chatham
    BORG_Problem_set_bounds(problem, 8, 0.0, 1.0); // Sanford intake expansion chatham low rank
    BORG_Problem_set_bounds(problem, 9, 0.0, 1.0); // Sanford intake expansion chatham high rank
 ...
 * infrastructure).
 */
int ChathamDefection::functionEvaluation(double *vars, double *objs, double *consts) {
//    cout << "Building Triangle Problem." << endl;
    // ===================== SET UP DECISION VARIABLES  =====================

    auto solutionDVs = Utils::parse2DCsvFile(io_directory + "Durham_best_DVs.csv");
    ...
    double chatham_restriction_trigger = vars[0];
    double chatham_transfer_trigger = vars[1];
    double Chatham_JLA = vars[2];
    double chatham_annual_payment = vars[3];
    double chatham_insurance_use = 1.1;
    double chatham_insurance_payment = 0;
    double chatham_inftrigger = vars[4];
    double chatham_inf_buffer = vars[5];
    //double western_wake_treatment_plant_chatham_frac = vars[6]; //0.296; // 29.6% of ultimate 2060 capacity
    double western_wake_treatment_plant_rank_chatham_low = vars[6];
    double western_wake_treatment_plant_rank_chatham_high = vars[7];
    double sanford_wtp_chatham_intake_expansion_low_rank = vars[8];
    double sanford_wtp_chatham_intake_expansion_high_rank = vars[9];
    ...
    double pittsboro_restriction_trigger = solutionDVs[0][39];
    double pittsboro_transfer_trigger = solutionDVs[0][40];
    double Pittsboro_JLA = solutionDVs[0][41];
    double pittsboro_annual_payment = solutionDVs[0][42];
    double pittsboro_insurance_use = 1.1;
    double pittsboro_insurance_payment = 0;
    double pittsboro_inftrigger = solutionDVs[0][43];
    double pittsboro_inf_buffer = solutionDVs[0][44];
    ...
    double western_wake_treatment_plant_pittsboro_frac = solutionDVs[0][51]; //0.167; // 16.7% of ultimate 2060 capacity
    double western_wake_treatment_plant_chatham_frac = solutionDVs[0][52]; //0.296; // 29.6% of ultimate 2060 capacity
    double western_wake_treatment_plant_rank_pittsboro_low = solutionDVs[0][53]; // 20
    double western_wake_treatment_plant_rank_pittsboro_high = solutionDVs[0][54]; // 21

    double sanford_wtp_pittsboro_intake_expansion_low_rank = solutionDVs[0][57];
    double sanford_wtp_pittsboro_intake_expansion_high_rank = solutionDVs[0][58];


    /// potential independent Pittsboro project(s)
    double haw_river_intake_expansion_rank_low = solutionDVs[0][61]; //0.9;
    double haw_river_intake_expansion_rank_high = solutionDVs[0][62]; //0.9;
...
//	printf("Function evaluation time: %f s\n", end_time - start_time);
    ...
    /// Calculate objectives and store them in Borg decision variables array.
        ...
        objs[0] = 1.0- objectives[24];
        objs[1] = objectives[25]; 
        objs[2] = objectives[26];
        objs[3] = objectives[27];
        objs[4] = objectives[28];
        objs[5] = objectives[29];
...
int ChathamDefection::simulationExceptionHander(const std::exception &e, Simulation *s,
...
ChathamDefection::~ChathamDefection() = default;

ChathamDefection::ChathamDefection(unsigned long n_weeks, int import_export_rof_table)
        : Problem(n_weeks) {
...
void ChathamDefection::readInputData() {
