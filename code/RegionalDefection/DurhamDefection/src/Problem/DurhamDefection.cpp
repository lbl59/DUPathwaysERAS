...
#ifdef PARALLEL
void DurhamDefection::setProblemDefinition(BORG_Problem &problem)
{
    // The parameter bounds are the same for all formulations
    BORG_Problem_set_bounds(problem, 0, 0.001, 1.0); // Durham restrictions
    BORG_Problem_set_bounds(problem, 1, 0.001, 1.0); // Durham Transfers
    BORG_Problem_set_bounds(problem, 2, 0.1, 0.35);  // Durham JLA
    BORG_Problem_set_bounds(problem, 3, 0.0, 0.1);  // Durham reserve fund cont
    BORG_Problem_set_bounds(problem, 4, 0.001, 1.0);  // Durham infrastructure trigger

    BORG_Problem_set_bounds(problem, 5, 0.0, 1.0);   // Teer quarry expansion ranking
    BORG_Problem_set_bounds(problem, 6, 0.0, 1.0);  // reclaimed water low
    BORG_Problem_set_bounds(problem, 7, 0.0, 1.0);  // reclaimed water high 
    BORG_Problem_set_bounds(problem, 8, 0.0, 1.0);  // Lake Michie expansion low ranking
    BORG_Problem_set_bounds(problem, 9, 0.0, 1.0);  // Lake Michie expansion high ranking
    BORG_Problem_set_bounds(problem, 10, 0.0, 1.0);  // WJLWTP rank Durham low
    BORG_Problem_set_bounds(problem, 11, 0.0, 1.0);  // WJLWTP rank Durham high
    //BORG_Problem_set_bounds(problem, 10, 0.001, 0.6);  // WJWLTP Durham frac
    BORG_Problem_set_bounds(problem, 12, 0.0, 20.0);  // Durham inf buffer
 ...
 * infrastructure).
 */
int DurhamDefection::functionEvaluation(double *vars, double *objs, double *consts) {
//    cout << "Building Triangle Problem." << endl;
    ...
    // Jan 2022: only Durham DVs are optimized under defection
    double Durham_restriction_trigger = vars[0];
    double durham_transfer_trigger = vars[1];
    double Durham_JLA = vars[2];
    double durham_annual_payment = vars[3]; // contingency fund
    double durham_inftrigger = vars[4];
    double Teer_quarry_expansion_ranking = vars[5]; // 9
    double reclaimed_water_ranking_low = vars[6]; // 18
    double reclaimed_water_high = vars[7]; // 19
    double lake_michie_expansion_ranking_low = vars[8]; // 15
    double lake_michie_expansion_ranking_high = vars[9]; // 16  
    double western_wake_treatment_plant_rank_durham_low = vars[10]; // 20
    double western_wake_treatment_plant_rank_durham_high = vars[11]; // 21  
    double durham_inf_buffer = vars[12];
...
//	printf("Function evaluation time: %f s\n", end_time - start_time);
    ...
    /// Calculate objectives and store them in Borg decision variables array.
        ...
        objs[0] = 1.0- objectives[6];
        objs[1] = objectives[7]; 
        objs[2] = objectives[8];
        objs[3] = objectives[9];
        objs[4] = objectives[10];
        objs[5] = objectives[11];
...
int DurhamDefection::simulationExceptionHander(const std::exception &e, Simulation *s,
...
DurhamDefection::~DurhamDefection() = default;

DurhamDefection::DurhamDefection(unsigned long n_weeks, int import_export_rof_table)
        : Problem(n_weeks) {
...
void DurhamDefection::readInputData() {
