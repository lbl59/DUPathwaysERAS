...
#ifdef PARALLEL
void OWASADefection::setProblemDefinition(BORG_Problem &problem)
{
    // The parameter bounds are the same for all formulations
    BORG_Problem_set_bounds(problem, 0, 0.001, 1.0); // OWASA restrictions
    BORG_Problem_set_bounds(problem, 1, 0.001, 1.0); // OWASA Transfers
    BORG_Problem_set_bounds(problem, 2, 0.05, 0.1);  // OWASA JLA
    BORG_Problem_set_bounds(problem, 3, 0.0, 0.1);  // OWASA reserve fund cont
    BORG_Problem_set_bounds(problem, 4, 0.001, 1.0);  // OWASA infrastructure trigger
    BORG_Problem_set_bounds(problem, 5, 0.0, 1.0);   // University Lake expansion ranking
    BORG_Problem_set_bounds(problem, 6, 0.0, 1.0);  // Cane creek expansion ranking
    BORG_Problem_set_bounds(problem, 7, 0.0, 1.0);  // Stone quarry shallow ranking
    BORG_Problem_set_bounds(problem, 8, 0.0, 1.0);  // Stone quarry deep ranking
    BORG_Problem_set_bounds(problem, 9, 0.0, 1.0);  // WJLWTP rank OWASA low
    BORG_Problem_set_bounds(problem, 10, 0.0, 1.0);  // WJLWTP rank OWASA high
    //BORG_Problem_set_bounds(problem, 11, 0.001, 0.2);  // WJWLTP OWASA frac
    BORG_Problem_set_bounds(problem, 11, 0.0, 20.0);  // OWASA inf buffer
 ...
 * infrastructure).
 */
int OWASADefection::functionEvaluation(double *vars, double *objs, double *consts) {
//    cout << "Building Triangle Problem." << endl;
    ...
    // December 2021: only OWASA DVs are optimized under defection
    double OWASA_restriction_trigger = vars[0];
    double owasa_transfer_trigger = vars[1];
    double OWASA_JLA = vars[2];
    double owasa_annual_payment = vars[3];
    double owasa_inftrigger = vars[4];
    double university_lake_expansion_ranking = vars[5]; // 14
    double Cane_creek_expansion_ranking = vars[6]; // 24
    double Stone_quarry_reservoir_expansion_shallow_ranking = vars[7]; // 12
    double Stone_quarry_reservoir_expansion_deep_ranking = vars[8]; // 13
    double western_wake_treatment_plant_rank_OWASA_low = vars[9]; // 20
    double western_wake_treatment_plant_rank_OWASA_high = vars[10]; // 21
    double owasa_inf_buffer = vars[11];
...
int OWASADefection::simulationExceptionHander(const std::exception &e, Simulation *s,
...
OWASADefection::~OWASADefection() = default;

OWASADefection::OWASADefection(unsigned long n_weeks, int import_export_rof_table)
        : Problem(n_weeks) {
...
void OWASADefection::readInputData() {
