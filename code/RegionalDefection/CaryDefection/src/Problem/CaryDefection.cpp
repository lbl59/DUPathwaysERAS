...
#ifdef PARALLEL
void CaryDefection::setProblemDefinition(BORG_Problem &problem)
{
    // The parameter bounds are the same for all formulations
    BORG_Problem_set_bounds(problem, 0, 0.001, 1.0); // Cary restrictions
    BORG_Problem_set_bounds(problem, 1, 0.39, 0.6);  // Cary JLA
    BORG_Problem_set_bounds(problem, 2, 0.0, 0.1);  // Cary reserve fund cont
    BORG_Problem_set_bounds(problem, 3, 0.001, 1.0);  // Cary infrastructure trigger
    BORG_Problem_set_bounds(problem, 4, 0.0, 20.0);  // Cary inf buffer
    BORG_Problem_set_bounds(problem, 5, 0.0, 1.0);  // Cary sanford wtp capacity stake rank
    BORG_Problem_set_bounds(problem, 6, 0.0, 1.0); // cape fear intake rank
 ...
 * infrastructure).
 */
int CaryDefection::functionEvaluation(double *vars, double *objs, double *consts) {
//    cout << "Building Triangle Problem." << endl;
    ...
    double cary_restriction_trigger = vars[0];
    double Cary_JLA = vars[1]; // is currently 0.462 fraction of WSP
    double cary_annual_payment = vars[2];
    double cary_inftrigger = vars[3];
    double cary_inf_buffer = vars[4];
    double cary_sanford_wtp_capacity_stake_rank = vars[5];
    double cape_fear_river_intake_rank = vars[6]; //0.8;
...
//	printf("Function evaluation time: %f s\n", end_time - start_time);
    ...
    /// Calculate objectives and store them in Borg decision variables array.
        ...
        objs[0] = 1.0- objectives[12];
        objs[1] = objectives[13]; 
        objs[2] = objectives[14];
        objs[3] = objectives[15];
        objs[4] = objectives[16];
        objs[5] = objectives[17];
...
int CaryDefection::simulationExceptionHander(const std::exception &e, Simulation *s,
...
CaryDefection::~CaryDefection() = default;

CaryDefection::CaryDefection(unsigned long n_weeks, int import_export_rof_table)
        : Problem(n_weeks) {
...
void CaryDefection::readInputData() {
