...
#ifdef PARALLEL
void PittsboroDefection::setProblemDefinition(BORG_Problem &problem)
{
    // The parameter bounds are the same for all formulations
    BORG_Problem_set_bounds(problem, 0, 0.001, 1.0); // pitt restriction trigger
    BORG_Problem_set_bounds(problem, 1, 0.001, 1.0); // pitt transfer trigger
    BORG_Problem_set_bounds(problem, 2, 0.06, 0.35); // pitt JLA
    BORG_Problem_set_bounds(problem, 3, 0.0, 0.1); // pitt annual payment
    BORG_Problem_set_bounds(problem, 4, 0.001, 1.0); // pitt inf trigger
    BORG_Problem_set_bounds(problem, 5, 0.0, 20.0); // pitt inf buff
    //BORG_Problem_set_bounds(problem, 6, 0.01, 0.5); // wjlwtp pit frac
    BORG_Problem_set_bounds(problem, 7, 0.0, 1.0); // wjlwtp rank low pitt
    BORG_Problem_set_bounds(problem, 8, 0.0, 1.0); // wjlwtp rank high pitt
    BORG_Problem_set_bounds(problem, 9, 0.0, 1.0); // Sanford intake expansion pitt low rank
    BORG_Problem_set_bounds(problem, 10, 0.0, 1.0); // Sanford intake expansion pitt high rank
    BORG_Problem_set_bounds(problem, 11, 0.0, 1.0); // haw intake expansion rank low
    BORG_Problem_set_bounds(problem, 12, 0.0, 1.0); // haw intake expansion rank high
    BORG_Problem_set_bounds(problem, 13, 0.0, 1.0); // cape fear intake rank
 ...
 * infrastructure).
 */
int PittsboroDefection::functionEvaluation(double *vars, double *objs, double *consts) {
//    cout << "Building Triangle Problem." << endl;
    // ===================== SET UP DECISION VARIABLES  =====================

    auto solutionDVs = Utils::parse2DCsvFile(io_directory + "compSol_DVs.csv");
    ...
    double pittsboro_restriction_trigger = vars[0];
    double pittsboro_transfer_trigger = vars[1];
    double Pittsboro_JLA = vars[2];
    double pittsboro_annual_payment = vars[3];
    double pittsboro_insurance_use = 1.1;
    double pittsboro_insurance_payment = 0;
    double pittsboro_inftrigger = vars[4];
    double pittsboro_inf_buffer = vars[5];
    //double western_wake_treatment_plant_pittsboro_frac = vars[6]; //0.167; // 16.7% of ultimate 2060 capacity
    double western_wake_treatment_plant_rank_pittsboro_low = vars[6]; // 20
    double western_wake_treatment_plant_rank_pittsboro_high = vars[7]; // 21
    double sanford_wtp_pittsboro_intake_expansion_low_rank = vars[8];
    double sanford_wtp_pittsboro_intake_expansion_high_rank = vars[9];
    double haw_river_intake_expansion_rank_low = vars[10]; //0.9;
    double haw_river_intake_expansion_rank_high = vars[11]; //0.9;
    


    double Durham_restriction_trigger = solutionDVs[0][0];
    double OWASA_restriction_trigger = solutionDVs[0][1];
    double raleigh_restriction_trigger = solutionDVs[0][2];
    double cary_restriction_trigger = solutionDVs[0][3];

    double durham_transfer_trigger = solutionDVs[0][4];
    double owasa_transfer_trigger = solutionDVs[0][5];
    double raleigh_transfer_trigger = solutionDVs[0][6];

    double Raleigh_JLA = 0;
    double OWASA_JLA = solutionDVs[0][7];
    double Durham_JLA = solutionDVs[0][8];
    double Cary_JLA = solutionDVs[0][9]; // is currently 0.462 fraction of WSP

    double durham_annual_payment = solutionDVs[0][10]; // contingency fund
    double owasa_annual_payment = solutionDVs[0][11];
    double raleigh_annual_payment = solutionDVs[0][12];
    double cary_annual_payment = solutionDVs[0][13];
    ...
    double durham_inftrigger = solutionDVs[0][14];
    double owasa_inftrigger = solutionDVs[0][15];
    double raleigh_inftrigger = solutionDVs[0][16];

    double university_lake_expansion_ranking = solutionDVs[0][17]; // 14
    double Cane_creek_expansion_ranking = solutionDVs[0][18]; // 24
    double Stone_quarry_reservoir_expansion_shallow_ranking = solutionDVs[0][19]; // 12
    double Stone_quarry_reservoir_expansion_deep_ranking = solutionDVs[0][20]; // 13
    double Teer_quarry_expansion_ranking = solutionDVs[0][21]; // 9
    double reclaimed_water_ranking_low = solutionDVs[0][22]; // 18
    double reclaimed_water_high = solutionDVs[0][23]; // 19
    double lake_michie_expansion_ranking_low = solutionDVs[0][24]; // 15
    double lake_michie_expansion_ranking_high = solutionDVs[0][25]; // 16
    double little_river_reservoir_ranking = solutionDVs[0][26]; // 7
    double richland_creek_quarry_rank = solutionDVs[0][27]; // 8
    double neuse_river_intake_rank = solutionDVs[0][28]; // 10
    double reallocate_falls_lake_rank = solutionDVs[0][29]; // 17

    double western_wake_treatment_plant_rank_OWASA_low = solutionDVs[0][30]; // 20
    double western_wake_treatment_plant_rank_OWASA_high = solutionDVs[0][31]; // 21
    double western_wake_treatment_plant_rank_durham_low = solutionDVs[0][32]; // 20
    double western_wake_treatment_plant_rank_durham_high = solutionDVs[0][33]; // 21
    ...
    double western_wake_treatment_plant_owasa_frac = solutionDVs[0][34];
    double western_wake_treatment_frac_durham = solutionDVs[0][35];
    double western_wake_treatment_plant_raleigh_frac = 0;

    double falls_lake_reallocation = 5637; // 17,300 AF

    double durham_inf_buffer = solutionDVs[0][36];
    double owasa_inf_buffer = solutionDVs[0][37];
    double raleigh_inf_buffer = solutionDVs[0][38];
    ...
    double chatham_restriction_trigger = solutionDVs[0][45];
    double chatham_transfer_trigger = solutionDVs[0][46];
    double Chatham_JLA = solutionDVs[0][47];
    double chatham_annual_payment = solutionDVs[0][48];
    double chatham_insurance_use = 1.1;
    double chatham_insurance_payment = 0;
    double chatham_inftrigger = solutionDVs[0][49];
    double chatham_inf_buffer = solutionDVs[0][50];
    ...
    double western_wake_treatment_plant_pittsboro_frac = solutionDVs[0][51];
    double western_wake_treatment_plant_chatham_frac = solutionDVs[0][52]; //0.296; // 29.6% of ultimate 2060 capacity

    double western_wake_treatment_plant_rank_chatham_low = solutionDVs[0][55]; // 20
    double western_wake_treatment_plant_rank_chatham_high = solutionDVs[0][56]; // 21

    double sanford_wtp_chatham_intake_expansion_low_rank = solutionDVs[0][59];
    double sanford_wtp_chatham_intake_expansion_high_rank = solutionDVs[0][60];
    ...
    double cape_fear_river_intake_rank = solutionDVs[0][63]; //0.8;
    ...
    int demand_projection_forecast_length = (int) solutionDVs[0][64];
    int demand_projection_historical_use = (int) solutionDVs[0][65];
    int demand_projection_frequency = (int) solutionDVs[0][66];
    ...
    double cary_inftrigger = solutionDVs[0][67];
    double cary_inf_buffer = solutionDVs[0][68];
    double cary_sanford_wtp_capacity_stake_rank = solutionDVs[0][69];
...
//                        added_capacity_sanford_wtp_intake_pittsboro_expansion_high);
        ...
        double western_wake_cost_frac_durham = western_wake_treatment_frac_durham / sum_wjlwtp;
        double western_wake_cost_frac_owasa = western_wake_treatment_plant_owasa_frac / sum_wjlwtp;
        double western_wake_cost_frac_pittsboro = western_wake_treatment_plant_pittsboro_frac / sum_wjlwtp;
        double western_wake_cost_frac_chatham = western_wake_treatment_plant_chatham_frac / sum_wjlwtp;
...
//	printf("Function evaluation time: %f s\n", end_time - start_time);
    ...
    /// Calculate objectives and store them in Borg decision variables array.
        ...
        objs[0] = 1.0- objectives[30];
        objs[1] = objectives[31]; 
        objs[2] = objectives[32];
        objs[3] = objectives[33];
        objs[4] = objectives[34];
        objs[5] = objectives[35];
...
int PittsboroDefection::simulationExceptionHander(const std::exception &e, Simulation *s,
...
PittsboroDefection::~PittsboroDefection() = default;

PittsboroDefection::PittsboroDefection(unsigned long n_weeks, int import_export_rof_table)
        : Problem(n_weeks) {
...
void PittsboroDefection::readInputData() {
